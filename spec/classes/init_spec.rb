...
describe 'sssd', :type => 'class' do
  ...
  context 'with backends => string' do
    ...
    it { expect { should compile }.to raise_error(/must be a hash/) }
  end
  ...
  context 'with filter_users => string' do
    let :params do
      {
        :domains       => ['value'],
        :filter_users  => 'string'
  ...
  context 'with filter_groups => string' do
    let :params do
      {
        :domains       => ['value'],
  ...
  context 'on a RedHat system' do
    ...
    context 'with domains => set, make_home_dir => true' do
      let :params do
        {
          :domains       => [ 'mydomain.local' ],
          :make_home_dir => true,
